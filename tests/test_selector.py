...
def _item(mid: str, ctx: int, params=None, modality="text") -> dict:
    return {
        "id": mid,
        "context_length": ctx,
...
def test_filter_free_only() -> None:
    items = [_item("a:free", 100000, ["tools"]), _item("b:paid", 100000, ["tools"])]
    out = apply_filters(items, {"free_only": True, "require_tools": True, "min_context": 0, "modality": "any"})
